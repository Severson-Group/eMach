...
class SPM_RotorStructuralAnalyzer:
    ...
    def DetermineCoeff(self, sh: "RotorComponent", rc, pm, sl, deltaT, omega):
        ...
        X[0] = (
            (3 * rc.C1 + rc.C2) * rc.Beta * (omega ** 2) * (r1 ** 2)
            + rc.zeta_r * deltaT
        ...
        X[1] = (
            (3 * pm.C1 + pm.C2) * pm.Beta * (omega ** 2) * (r2 ** 2)
            + pm.zeta_r * deltaT
        ...
        X[2] = (
            (3 * sl.C1 + sl.C2) * sl.Beta * (omega ** 2) * (r3 ** 2)
            + sl.zeta_r * deltaT
        ...
        X[3] = (
            -((3 * sl.C1 + sl.C2) * sl.Beta * (omega ** 2) * (r4 ** 2))
            - sl.zeta_r * deltaT
        )
        ...
        X[4] = (
            delta_1
            + rc.Beta * (omega ** 2) * (r1 ** 3)
            - (sh.Beta * (omega ** 2) * (r1 ** 3))
        )
...
class Material_Transverse_Isotropic:
    def __init__(
        ...
        self.E_p = (
            ElasticMod_Plane  # Elastic modulus of material in the plane direction
        )
    ...
    @property
    def Del(self):
        return ((1 + self.Nu_p) * (1 - self.Nu_p - 2 * self.Nu_pt * self.Nu_tp)) / (
            (self.E_p ** 2) * self.E_t
        )
    ...
    def zeta_r(self):
        return -(
            (((self.C1 - self.C2) / (self.C1 - self.C3)) - self.C1) * self.alpha_r
    ...
    def zeta_t(self):
        return -(
            (((self.C1 - self.C2) / (self.C1 - self.C3)) - self.C2) * self.alpha_r
    ...
    def zeta_u(self):
        return -(
            (((self.C1 - self.C2) / (self.C1 - self.C3))) * self.alpha_r
...
class Sigma:
    ...
    def radial(self, R):
        ...
        sigma_r = (
            ...
            * (np.power(R, (self.rotComp.h - 1)))
            + self.A[1]
            * (self.rotComp.C2 - self.rotComp.C1 * self.rotComp.h)
            * (np.power(R, (-self.rotComp.h - 1)))
            + (3 * self.rotComp.C1 + self.rotComp.C2)
            * self.rotComp.Beta
            * (self.omega ** 2)
            * (np.power(R, 2))
            + self.rotComp.zeta_r * self.deltaT
        )
        # Tangential Stress
        return sigma_r
    ...
    def tangential(self, R):
        ...
        sigma_t = (
            ...
            * (np.power(R, (self.rotComp.h - 1)))
            + self.A[1]
            * (self.rotComp.C3 - self.rotComp.C2 * self.rotComp.h)
            * (np.power(R, (-self.rotComp.h - 1)))
            + (3 * self.rotComp.C2 + self.rotComp.C3)
            * self.rotComp.Beta
            * (self.omega ** 2)
            * (np.power(R, 2))
            + self.rotComp.zeta_t * self.deltaT
        )
        return sigma_t
...
class SleeveAnalyzer:
    ...
    def analyze(self, problem: "SleeveProblem"):
        ...
        nlc2 = op.NonlinearConstraint(
            problem.tan_sleeve, -np.inf, self.stress_limits["tan_sleeve"]
        )
        nlc3 = op.NonlinearConstraint(
            problem.rad_magnet, -np.inf, self.stress_limits["rad_magnets"]
        )
        nlc4 = op.NonlinearConstraint(
            problem.tan_magnet, -np.inf, self.stress_limits["tan_magnets"]
        )
