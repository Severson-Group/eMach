# -*- coding: utf-8 -*-
"""
Created on Sat Jul 24 10:28:52 2021
...
class AirflowAnalyzer:
    ...
    def analyze(self,problem):
        nlc1 = op.NonlinearConstraint(problem.magnet_temp,
                                      0,
                                      self.max_temp)
        const=nlc1
        ...
        print(sol)
        if sol.success == True:
            results={'message':sol.success,'magnet Temp':problem.magnet_temp(sol.x),'Required Airflow':sol.x}
            return results
        else:
            results={'message':sol.success,'magnet Temp':problem.magnet_temp(sol.x),'Required Airflow':sol.x}
...
class WindageProblem:
    def __init__(self,Omega, R_ro ,stack_length,R_st,air_gap,m_dot_air, TEMPERATURE_OF_AIR=25):
        self.Omega=Omega
        self.R_ro=R_ro
...
class WindageLossAnalyzer:
    ...
    def analyze(self,problem):
        ...
        #Num_shaft_section = 1
        T_Air = problem.TEMPERATURE_OF_AIR #20:(120-20)/((SpeedMax-SpeedMin)/SpeedStep):120         #; % Air temperature []
        ...
        windage_loss_axial = (2/3) * np.pi * rho_Air * (problem.R_st**3 - (problem.R_ro)**3) * vm * um * problem.Omega
    ...
    def h(self):
        ...
        if Ta_m <=41:
            ...
            if self.u_z==0:
                ...
                self.Nu=(.022*(1+D_h*u_theta/(np.pi*a*self.u_z)**2)**.8714)*(Re_z**.8)*(Pr**.5)
        else:
            self.Nu=None
