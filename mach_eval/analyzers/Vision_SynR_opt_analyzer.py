...
class Vision_SynR_Opt_Problem:
    ...
    def _validate_attr(self):
        if 'Vision_SynR_Machine' in str(type(self.machine)):
            pass
        else:
            raise TypeError("Invalid machine type")

        if 'Vision_SynR_Machine_Oper_Pt' in str(type(self.operating_point)):
            pass
        else:
...
class Vision_SynR_Opt_Analyzer:
    ...
    def draw_machine(self, tool):
        ...
        self.rotor_barrier_1 = mo.CrossSectFluxBarrierRotorPartialBarrier1(
            name="RotorBarrier1",
            dim_alpha_b=mo.DimDegree(self.machine_variant.alpha_b),
            dim_r_ri=mo.DimMillimeter(self.machine_variant.r_ri),
            dim_r_ro=mo.DimMillimeter(self.machine_variant.r_ro),
        ...
        self.rotor_barrier_2 = mo.CrossSectFluxBarrierRotorPartialBarrier2(
            name="RotorBarrier2",
            dim_alpha_b=mo.DimDegree(self.machine_variant.alpha_b),
            dim_r_ri=mo.DimMillimeter(self.machine_variant.r_ri),
            dim_r_ro=mo.DimMillimeter(self.machine_variant.r_ro),
        ...
        self.comp_rotor_barrier_1 = mo.Component(
            name="RotorBarrier1",
            cross_sections=[self.rotor_barrier_1],
        ...
        self.comp_rotor_barrier_2 = mo.Component(
            name="RotorBarrier2",
            cross_sections=[self.rotor_barrier_2],
            material=mo.MaterialGeneric(name=self.machine_variant.rotor_barrier_mat["rotor_barrier_material"], color=r"#71797E"),
            make_solid=mo.MakeExtrude(location=mo.Location3D(), 
                    dim_depth=mo.DimMillimeter(self.machine_variant.l_st)),
            )
        ...
        tool.sketch = tool.create_sketch()
        tool.sketch.SetProperty("Name", self.rotor_barrier_1.name)
        tool.sketch.SetProperty("Color", r"#71797E")
        cs_rotor_barrier_1 = self.rotor_barrier_1.draw(tool)
        rotor_tool = tool.prepare_section(cs_rotor_barrier_1, 2*self.machine_variant.p)

        tool.sketch = tool.create_sketch()
        tool.sketch.SetProperty("Name", self.rotor_barrier_2.name)
        tool.sketch.SetProperty("Color", r"#71797E")
        cs_rotor_barrier_2 = self.rotor_barrier_2.draw(tool)
        rotor_tool = tool.prepare_section(cs_rotor_barrier_2, 2*self.machine_variant.p)
    ...
    def pre_process(self, model):
        ...
        if len(part_ID_list) != int(
            1 + 1 + self.machine_variant.Q * 2 + 1 + self.machine_variant.p * 2 + self.machine_variant.p * 2
        ):
        ...
        self.id_rotorCore = id_rotorCore = part_ID_list[int(2 * self.machine_variant.Q + 1) : int(2 * self.machine_variant.Q + 10) ]
    ...
    def create_rotor_iron_material(self, app, steel_name):
        ...
        app.GetMaterialLibrary().GetUserMaterial(
            ...
            "PoissonRatio", self.machine_variant.rotor_iron_mat["rotor_iron_poission_ratio"]
        )
        app.GetMaterialLibrary().GetUserMaterial(
            self.machine_variant.rotor_iron_mat["rotor_iron_material"]
        ).SetValue(
            "ShearModulus", self.machine_variant.rotor_iron_mat["rotor_iron_shear_modulus"] / 1000000
        )
    ...
    def add_materials(self, study):
        ...
        study.SetMaterialByName(self.rotor_core.name, 
            self.machine_variant.rotor_iron_mat["rotor_iron_material"])
        study.GetMaterial(self.rotor_core.name).SetValue("Laminated", 1)
        study.GetMaterial(self.rotor_core.name).SetValue("LaminationFactor",
            self.machine_variant.rotor_iron_mat["rotor_iron_stacking_factor"])
        
        study.SetMaterialByName(self.rotor_barrier_1.name,
            self.machine_variant.rotor_barrier_mat["rotor_barrier_material"])
        study.GetMaterial(self.rotor_barrier_1.name).SetValue("Laminated", 0)

        study.SetMaterialByName(self.rotor_barrier_2.name,
            self.machine_variant.rotor_barrier_mat["rotor_barrier_material"])
        study.GetMaterial(self.rotor_barrier_2.name).SetValue("Laminated", 0)
