...
class CrossSectInnerRotorStator:
    ...
    def __init__(
        ...
        deg_alpha_so=20,  # span angle of tooth: class type DimAngular
    ...
    def draw(self, drawer):
        ...
        r_syi = r_si + d_sp
        w_st_half = w_st * 0.5
        ang = arctan(w_st_half / r_syi)
        P4 = [r_syi * cos(ang), r_syi * -sin(ang)]

        P5 = [P4[0] + d_st, P4[1]]

        P6 = [
            (r_si + d_sp + d_st) * cos(alpha_slot_span * 0.5),
            (r_si + d_sp + d_st) * -sin(alpha_slot_span * 0.5),
        ]

        P7 = [
            (r_si + d_sp + d_st + d_sy) * cos(alpha_slot_span * 0.5),
...
class CrossSectInnerRotorStatorWinding(object):
    ...
    def draw(self, drawer, bool_re_evaluate=False):
        ...
        POpen = [
            (r_si + d_sp) * cos(alpha_slot_span * 0.5 * 1.00),
        ...
        P6 = [
            (r_si + d_sp + d_st) * cos(alpha_slot_span * 0.5) * 1.00,
...
if __name__ == "__main__":
    ...
    stator_core = CrossSectInnerRotorStator(
        name="StatorCore",
        deg_alpha_st=40,
        deg_alpha_so=20,
        mm_r_si=40,
    ...
    coils = CrossSectInnerRotorStatorWinding(name="Coils", stator_core=stator_core)
