...
As mentioned in the previous sub-step, the ``Analyzer`` protocol is designed to allow for modular and generalized analysis code that can be re-used to study multiple machines. It usually makes sense to create ``Analyzers`` for code that is lengthy enough to warrant the added complexity over the ``EvaluationStep`` protocol, or be code which is likely to be used by other developers or to study multiple machine types. For this example, the ``Analyzer`` is simple and could have been defined as an ``EvaluationStep`` but is defined this way to demonstrate how to implement the relevant classes. 
