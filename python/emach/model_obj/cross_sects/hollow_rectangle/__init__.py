...
class CrossSectHollowRect(CrossSectBase):
    ...
    def draw(self, drawer):
        ...
        inner_pts = np.array([x_inner, y_inner])
        outer_pts = np.array([x_outer, y_outer])
        
        inner_pts = np.transpose(inner_pts)# convert coordinates to a form of [[x1,y1]..]
        outer_pts = np.transpose(outer_pts)
