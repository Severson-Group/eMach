...
if __name__ == '!__main__':
    ...
    def parallel_label(vg, label, PA, PB, vector_extendion=None, translation=0, rotation=0, distance_factor=2,
                       mirror=False):
...
# winding diagram for two/four pole dpnv motor
if __name__ == '__main__':
    ...
    if bool_4pole_2pole:  # 4 pole motor
        # ExampleQ24p2m3ps1y6
        l41 = ['C', 'C', 'A', 'A', 'B', 'B', 'C', 'C', 'A', 'A', 'B', 'B', 'C', 'C', 'A', 'A', 'B', 'B', 'C', 'C', 'A',
               'A', 'B', 'B']
        l42 = ['+', '+', '-', '-', '+', '+', '-', '-', '+', '+', '-', '-', '+', '+', '-', '-', '+', '+', '-', '-', '+',
               '+', '-', '-']
        ...
        l_rightlayer1 = ['C', 'A', 'A', 'A', 'A', 'B', 'B', 'B', 'B', 'C', 'C', 'C', 'C', 'A', 'A', 'A', 'A', 'B', 'B',
                         'B', 'B', 'C', 'C', 'C'][::-1]
        l_rightlayer2 = ['-', '+', '+', '+', '+', '-', '-', '-', '-', '+', '+', '+', '+', '-', '-', '-', '-', '+', '+',
                         '+', '+', '-', '-', '-'][::-1]
        l_leftlayer1 = ['C', 'C', 'C', 'C', 'A', 'A', 'A', 'A', 'B', 'B', 'B', 'B', 'C', 'C', 'C', 'C', 'A', 'A', 'A',
                        'A', 'B', 'B', 'B', 'B'][::-1]
        l_leftlayer2 = ['-', '-', '-', '-', '+', '+', '+', '+', '-', '-', '-', '-', '+', '+', '+', '+', '-', '-', '-',
                        '-', '+', '+', '+', '+'][::-1]
    ...
    def draw_vertical_line_and_arrow_head(x, x2):
        ...
        ax.text(xy[0] + 1.25 * coil_bias, annotate_bias + 0.25, '%d' % (abs(x)), fontsize=9)
        ...
        ax.annotate('', xytext=(xytext[0] + coil_bias, xytext[1]), xy=(xy[0] + coil_bias, xy[1]), xycoords='data',
                    arrowprops=dict(arrowstyle="->", color=color, alpha=1.0))
    ...
    def draw_terminals(list_terminals, slope, phase='u'):
        ...
        for xx, letter in zip(list_terminals, ['b', 'a', 'd', 'c']):  # ['a', 'b', 'c', 'd'] changed. 6/26/2019
            ...
            ax.annotate('', xytext=xytext, xy=xy, xycoords='data',
                        arrowprops=dict(arrowstyle="->", color=color, lw=0.5, alpha=1.0))
            ...
            ax.text(c1[0] - 0.5, c1[1] + 0.5, terminal_symbols[0], fontsize=9, color=color)
            ax.text(c2[0] - 0.5, c2[1] + 0.5, terminal_symbols[1], fontsize=9, color=color)
    ...
    for phase in ['u', 'v', 'w']:
        ...
        ax = fig.add_subplot(111,
                             aspect='equal')  # fixed aspect ratio such that a circle is still a circle when you drag the tk plot window
        ax.axis('off')
        ...
        for color, winding_layout_right, winding_layout_left, _ in zip(['#5C9C31', '#E97675', '#6C8FAB'],
                                                                       [dl_rightlayer['A'], dl_rightlayer['B'],
                                                                        dl_rightlayer['C']],
                                                                       [dl_leftlayer['A'], dl_leftlayer['B'],
                                                                        dl_leftlayer['C']],
                                                                       list(range(3))):
            # if color != 'b':
            #     continue
        ...
        if bool_4pole_2pole:  # 4 pole motor
            ...
            if phase == 'v':
                color = '#E97675'
                list_terminals = [(9 + Qs / 3, -(10 + Qs / 3)), (-(4 + Qs / 3), 3 + Qs / 3),
                                  (-(16 + Qs / 3), 15 + Qs / 3), (21 + Qs / 3 - Qs, -(22 + Qs / 3 - Qs))]
            if phase == 'u':
                color = '#6C8FAB'
                list_terminals = [(9 - Qs / 3, -(10 - Qs / 3)), (-(4 - Qs / 3 + Qs), 3 - Qs / 3 + Qs),
                                  (-(16 - Qs / 3), 15 - Qs / 3), (21 - Qs / 3, -(22 - Qs / 3))]
            ...
            if phase == 'v':
                color = '#E97675'
                list_terminals = [(19 + Qs / 3, -(20 + Qs / 3)), (-(10 + Qs / 3), 9 + Qs / 3),
                                  (21 + Qs / 3, -(22 + Qs / 3)),
                                  (-(8 + Qs / 3), 7 + Qs / 3)]  # we have swapped the order of ub and ud
            if phase == 'u':
                color = '#6C8FAB'
                list_terminals = [(19 - Qs / 3, -(20 - Qs / 3)), (-(10 - Qs / 3), 9 - Qs / 3),
                                  (21 - Qs / 3, -(22 - Qs / 3)),
                                  (-(8 - Qs / 3 + Qs), 7 - Qs / 3 + Qs)]  # we have swapped the order of ub and ud
    ...
    class one_phase_winding(object):
        ...
        def draw_arrow(self):
            ...
            for x, label in zip(self.list_slot_location, self.list_labels):
                ...
                ax.annotate('', xytext=xytext, xy=xy, xycoords='data',
                            arrowprops=dict(arrowstyle="->", color=self.color, alpha=1.0))
    ...
    if False:
        ...
        for phase in list_phase_name:
            ...
            one_phase_winding(ax, [number * direction for number, direction in
                                   zip(eval(f'{phase}'), eval(f'{phase}_direction'))])
    else:
