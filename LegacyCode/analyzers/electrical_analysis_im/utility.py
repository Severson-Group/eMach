...
def myLogger(dir_log,
             prefix='default_prefix_'):  # This works even when the module is reloaded (which is not the case of the other answers) https://stackoverflow.com/questions/7173033/duplicate-log-output-when-using-python-logging-module
...
def singleSidedDFT(signal, samp_freq):
    ...
    dft_single_sided = [2 * abs(dft_bin) / NFFT for dft_bin in dft_complex][0:int(
        NFFT / 2) + 1]  # /NFFT for spectrum aplitude consistent with actual signal. 2* for single-sided. abs for amplitude of complem number.
    dft_single_sided[0] *= 0.5  # DC bin in single sided spectrem does not need to be times 2
    return np.array(dft_single_sided)
    dft_freq = 0.5 * samp_freq * np.linspace(0, 1,
                                             NFFT / 2 + 1)  # unit is Hz # # f = np.fft.fftfreq(NFFT, Ts) # for double-sided
...
class Pyrhonen_design(object):
    def __init__(self, im, bounds=None):
        ...
        self.rotor_tooth_width_b_dr = (2 * pi * (
                    im.Radius_OuterRotor - im.Length_HeadNeckRotorSlot) - im.Radius_of_RotorSlot * (
                                                   2 * Qr + 2 * pi)) / Qr
...
# Evaluation Utility
# ~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~
def add_plots(axeses, dm, title=None, label=None, zorder=None, time_list=None, sfv=None, torque=None, range_ss=None,
              alpha=0.7):
    info = '%s' % (title)
    ...
    info += '\nNormalized Force Error Mag: %g%%, (+)%g%% (-)%g%%' % (normalized_force_error_magnitude * 100,
                                                                     sfv.ss_max_force_err_abs[
                                                                         0] / sfv.ss_avg_force_magnitude * 100,
                                                                     sfv.ss_max_force_err_abs[
                                                                         1] / sfv.ss_avg_force_magnitude * 100)
    ...
    if axeses is not None:
        ...
        ax = axeses[0][0];
        ax.plot(time_list, torque, alpha=alpha, label=label, zorder=zorder)
        ax.plot(time_list, np.ones(len(time_list)) * torque_average, 'k-')
        ax = axeses[0][1];
        ax.plot(time_list, sfv.force_abs, alpha=alpha, label=label, zorder=zorder)
        ...
        ax = axeses[1][0];
        ax.plot(time_list, 100 * sfv.force_err_abs / sfv.ss_avg_force_magnitude, label=label, alpha=alpha,
                zorder=zorder)
        ax = axeses[1][1];
...
def build_str_results(axeses, acm_variant, project_name, tran_study_name, dir_csv_output_folder, fea_config_dict,
                      femm_solver=None, machine_type=None):
    # originate from fobj

    try:
        dm = read_csv_results_4_general_purpose(tran_study_name, dir_csv_output_folder, fea_config_dict, femm_solver,
                                                machine_type=machine_type, acm_variant=acm_variant)
    except Exception as e:
        print(e)
        logging.getLogger(__name__).error(
            'Error when loading csv results for Tran2TSS. Check the Report of JMAG Designer. (Maybe Material is not added.)',
            exc_info=True)
        msg = 'CSV results are not found. Will re-build and re-run the JMAG project...'
        raise ExceptionReTry(msg)
        # return None
        # raise e
    ...
    sfv = suspension_force_vector(ForConX_list, ForConY_list,
                                  range_ss=number_of_steps_at_steady_state)  # samples in the tail that are in steady state
    ...
    if dm.jmag_loss_list is None:
        ...
        str_results += '\n\tjmag loss info: ' + ', '.join(['%g' % (el) for el in
                                                           dm.jmag_loss_list])  # dm.jmag_loss_list = [stator_copper_loss, rotor_copper_loss, stator_iron_loss, stator_eddycurrent_loss, stator_hysteresis_loss]

    str_results += '\n\tfemm loss info: ' + ', '.join(['%g' % (el) for el in dm.femm_loss_list])

    if fea_config_dict['delete_results_after_calculation'] == False:
        power_factor = dm.power_factor(number_of_steps_at_steady_state, targetFreq=acm_variant.DriveW_Freq)
        str_results += '\n\tPF: %g' % (power_factor)
    ...
    machine_results = [power_factor, efficiency, torque_average, normalized_torque_ripple, ss_avg_force_magnitude,
                       normalized_force_error_magnitude, force_error_angle]
    ...
    str_machine_results = ','.join('%g' % (el) for el in machine_results if
                                   el is not None)  # note that femm_loss_list can be None called by release_design.py

    cost_function_O1, list_cost_O1 = compute_list_cost(use_weights('O1'), rotor_volume, rotor_weight, torque_average,
                                                       normalized_torque_ripple, ss_avg_force_magnitude,
                                                       normalized_force_error_magnitude, force_error_angle,
                                                       dm.jmag_loss_list, dm.femm_loss_list, power_factor, total_loss)
    cost_function_O2, list_cost_O2 = compute_list_cost(use_weights('O2'), rotor_volume, rotor_weight, torque_average,
                                                       normalized_torque_ripple, ss_avg_force_magnitude,
                                                       normalized_force_error_magnitude, force_error_angle,
                                                       dm.jmag_loss_list, dm.femm_loss_list, power_factor, total_loss)
    ...
    speed_rpm = acm_variant.spec_input_dict['ExcitationFreqSimulated'] * 60 / acm_variant.spec_input_dict['p']  # rpm
    required_torque = acm_variant.spec_input_dict['mec_power'] / (2 * np.pi * speed_rpm) * 60
    ...
    if 'IM' in machine_type:
        ...
        stator_current_density = dm.ui_info[2] / 1.4142135623730951 / (
                    acm_variant.coils.mm2_slot_area * 1e-6 / acm_variant.DriveW_zQ)
    ...
    rated_shaft_power = acm_variant.Omega * required_torque
    ...
    list_weighted_ripples = [normalized_torque_ripple / 0.05, normalized_force_error_magnitude / 0.05,
                             force_error_angle]
    ...
    if 'IM' in machine_type:
        ...
        # Vol_Fe = (2*acm_variant.Radius_OuterStatorYoke*1e-3) ** 2 * (rated_stack_length_mm*1e-3) # 注意，硅钢片切掉的方形部分全部消耗了。# Option 1 (Jiahao)
        Vol_Fe = (np.pi * (acm_variant.Radius_OuterStatorYoke * 1e-3) ** 2 - np.pi * (
                    acm_variant.Radius_InnerRotor * 1e-3) ** 2) * (rated_stack_length_mm * 1e-3)  # Option 2 (Eric)
    ...
    print('FRW:', FRW, 'Rotor weight:', rotor_weight, 'Stack length:', acm_variant.stack_length, 'Rated stack length:',
    ...
    str_results = '\n-------\n%s-%s\n%d,%d,O1=%g,O2=%g,f1=%g,f2=%g,f3=%g\n%s\n%s\n%s\n' % (
        ...
        acm_variant.number_current_generation, acm_variant.individual_index, cost_function_O1, cost_function_O2, f1, f2,
        f3,
        str_machine_results,
    ...
    return (cost_function_O1,
            cost_function_O2), f1, f2, f3, FRW, normalized_torque_ripple, normalized_force_error_magnitude, force_error_angle, \
           project_name, acm_variant.get_individual_name(), \
           acm_variant.number_current_generation, acm_variant.individual_index, \
           power_factor, \
           rated_ratio, \
           rated_stack_length_mm, \
           rated_total_loss, \
...
def pyplot_clear(axeses):
    ...
    for ax in [axeses[0][0], axeses[0][1], axeses[1][0], axeses[1][1]]:
        ...
        ax.grid()
    ax = axeses[0][0];
    ax.set_xlabel('(a)', fontsize=14.5);
    ax.set_ylabel('Torque [Nm]', fontsize=14.5)
    ax = axeses[0][1];
    ax.set_xlabel('(b)', fontsize=14.5);
    ax.set_ylabel('Force Amplitude [N]', fontsize=14.5)
    ax = axeses[1][0];
    ax.set_xlabel('Time [s]\n(c)', fontsize=14.5);
    ax.set_ylabel('Norm. Force Error Mag. [%]', fontsize=14.5)
    ax = axeses[1][1];
    ax.set_xlabel('Time [s]\n(d)', fontsize=14.5);
    ax.set_ylabel('Force Error Angle [deg]', fontsize=14.5)
...
def collect_jmag_Tran2TSSProlong_results(im_variant, path_prefix, fea_config_dict, axeses, femm_solver_data=None):
    ...
    if fea_config_dict['number_cycles_prolonged'] != 0:
        sfv = suspension_force_vector(ForConX_list, ForConY_list,
                                      range_ss=400)  # samples in the tail that are in steady state
        str_results, torque_average, normalized_torque_ripple, ss_avg_force_magnitude, normalized_force_error_magnitude, force_error_angle = \
            add_plots(axeses, dm,
        ...
        # print str_results
        data_results.extend(
            [torque_average, normalized_torque_ripple, ss_avg_force_magnitude, normalized_force_error_magnitude,
             force_error_angle])
    else:
    ...
    # Tran2TSS
    ################################################################
    number_of_total_steps = 1 + fea_config_dict['number_of_steps_1stTSS'] + fea_config_dict[
        'designer.number_of_steps_2ndTSS']
    ...
    sfv = suspension_force_vector(ForConX_list, ForConY_list, range_ss=fea_config_dict[
        'designer.number_of_steps_2ndTSS'])  # samples in the tail that are in steady state
    str_results, torque_average, normalized_torque_ripple, ss_avg_force_magnitude, normalized_force_error_magnitude, force_error_angle = \
        add_plots(axeses, dm,
    ...
    if femm_solver_data is not None:
        ...
        sfv = suspension_force_vector(femm_force_x, femm_force_y, range_ss=len(
            rotor_position_in_deg))  # samples in the tail that are in steady state
        str_results, torque_average, normalized_torque_ripple, ss_avg_force_magnitude, normalized_force_error_magnitude, force_error_angle = \
            add_plots(axeses, dm,
                      title=study_name,
...
class data_manager(object):
    ...
    def power_factor(self, number_of_steps_at_steady_state, targetFreq=1e3, numPeriodicalExtension=1000):
        ...
        # plot(mytime, current)
        # show()
        power_factor, u, i, phase_diff_ui = compute_power_factor_from_half_period(voltage, current, mytime,
...
def get_copper_loss_Bolognani(stator_slot_area, rotor_slot_area=None, STATOR_SLOT_FILL_FACTOR=0.5,
                              ROTOR_SLOT_FILL_FACTOR=1.0, TEMPERATURE_OF_COIL=75, copper_loss_parameters=None):
    ...
    kov = 1.8  # \in [1.6, 2.0]
    end_winding_length_Lew = np.pi * 0.5 * (slot_pitch_pps + tooth_width_w_t) + slot_pitch_pps * kov * (
                coil_pitch_yq - 1)
...
def read_csv_results_4_general_purpose(study_name, path_prefix, fea_config_dict, femm_solver, machine_type=None,
                                       acm_variant=None):
            ...
            for row in csv_row_reader(f):
                ...
                if count == 1:  # Time | Terminal1 | Terminal2 | ... | Termial6
                    if 'Time' in row[0]:  # Time, s
                        for key in row:
                            new_key_list.append(
                                key)  # Yes, you have to use a new key list, because the ind below bgeins at 0.
                            Current_dict[key] = []
                    else:
    ...
    with open(path_prefix + study_name + '_joule_loss.csv', 'r') as f:
        ...
        for row in csv_row_reader(f):
            ...
            if 'IM' in machine_type:
                if count == 8:
                    headers = row
                    for idx_coil, h in enumerate(
                            headers):  # on server there are 3 air regions... while on PC there are 2...
                        if 'Coil' in h:
                            break
                ...
                if count > 8:
                    if count == 8 + 1:
                        ...
                        stator_copper_loss = float(
                            row[idx_coil])  # Coil # it is the same over time, this value does not account for end coil
    ...
    effective_part = rotor_Joule_loss_list[-int(
        0.5 * fea_config_dict['designer.number_of_steps_2ndTSS']):]  # number_of_steps_2ndTSS = steps for half peirod
    ...
    if femm_solver is not None:
        ...
        try:
            ...
            femm_solver.list_rotor_current_amp = [abs(el) for el in
                                                  femm_solver.vals_results_rotor_current]  # el is complex number
            ...
            _s, _r, _sAlongStack, _rAlongStack, _Js, _Jr = femm_solver.get_copper_loss_pyrhonen(
        ...
        copper_loss_parameters = [acm_variant.sleeve_length + acm_variant.fixed_air_gap_length,
                                  ...
                                  acm_variant.wily.coil_pitch_y,
                                  acm_variant.Q,
                                  acm_variant.stack_length,
                                  acm_variant.DriveW_CurrentAmp + acm_variant.BeariW_CurrentAmp,
        ...
        #     print('Heads up! slot_area_utilizing_ratio is', slot_area_utilizing_ratio, 'which means you are simulating a separate winding? If not, contrats--you found a bug...')
        #     print('DW, BW, Total:', acm_variant.DriveW_CurrentAmp, acm_variant.BeariW_CurrentAmp, acm_variant.CurrentAmp_per_phase)
        s, r, sAlongStack, rAlongStack, Js, Jr, Vol_Cu = get_copper_loss_Bolognani(
            acm_variant.slot_area_utilizing_ratio * acm_variant.coils.mm2_slot_area * 1e-6,
...
class Goertzel_Data_Struct(object):
    ...
    def goertzel_realtime(gs, targetFreq, numSamples, samplingRate, data):
        ...
        try:
            ...
            raise Exception(
                'This is for real time implementation of Goertzel, hence data must be a scalar rather than array-like object.')
...
def compute_list_cost(weights, rotor_volume, rotor_weight, torque_average, normalized_torque_ripple,
                      ss_avg_force_magnitude, normalized_force_error_magnitude, force_error_angle, jmag_loss_list,
                      femm_loss_list, power_factor, total_loss):
    # O2
...
def fobj_scalar(torque_average, ss_avg_force_magnitude, normalized_torque_ripple, normalized_force_error_magnitude,
                force_error_angle, total_loss,
    ...
    list_cost = [30e3 / (torque_average / rotor_volume),
                 ...
                 # [deg] /5 deg * 0.1 is reported to be the base line (Yegu Kang) # force_error_angle is not consistent with Yegu Kang 2018-060-case of TFE
                 total_loss / 2500.]  # 10 / efficiency**2,
    cost_function = np.dot(np.array(list_cost), np.array(weights))
    return cost_function


def fobj_list(l_torque_average, l_ss_avg_force_magnitude, l_normalized_torque_ripple,
              l_normalized_force_error_magnitude, l_force_error_angle, l_total_loss,
              weights=None, rotor_volume=None, rotor_weight=None):
    l_cost_function = []
    for torque_average, ss_avg_force_magnitude, normalized_torque_ripple, normalized_force_error_magnitude, force_error_angle, total_loss in zip(
            l_torque_average, l_ss_avg_force_magnitude, l_normalized_torque_ripple, l_normalized_force_error_magnitude,
            l_force_error_angle, l_total_loss):
        list_cost = [30e3 / (torque_average / rotor_volume),
                     normalized_torque_ripple * 20,
                     1.0 / (ss_avg_force_magnitude / rotor_weight),
                     normalized_force_error_magnitude * 20,
...
class SwarmDataAnalyzer(object):
        ...
        for i in range(self.number_of_designs)[::-1]:
            ...
            if the_row[0] == generation_index:
                if the_row[1] == individual_index:
                    ...
                    return [float(el) for el in self.buf[i * 21:(1 + i) * 21][5].split(',')], float(
                        self.buf[i * 21:(1 + i) * 21][2].split(',')[2])
        return None, None
    ...
    def my_scatter_plot(self, x, y, O, fig=None, ax=None, s=15, marker='.',
                        index_list=None):  # index_list is for filtered case
        O1, O2 = None, None
        ...
        if True:
            ...
            if index_list is not None:
                ...
                self.best_design_display = next(
                    itertools.islice(self.design_display_generator(), index_list[best_index], None))
                self.best_design_denorm = next(
                    itertools.islice(self.design_parameters_generator(), index_list[best_index], None))
    ...
    def pareto_plot_torque_force(self, fig2, axeses, marker='.'):
        ...
        O = fobj_list(list(self.get_certain_objective_function(2)),
                      ...
                      list(self.get_certain_objective_function(6)),
                      list(self.get_certain_objective_function(15)),
                      weights=self.weights_used, rotor_volume=self.rotor_volume,
                      rotor_weight=self.rotor_weight).tolist()
        ...
        if True:
            ...
            ax = axeses[0][0]
            x, y = np.array(list(self.get_certain_objective_function(2))) / self.rotor_volume / 1e3, list(
                self.get_certain_objective_function(3))
            x = x.tolist()
            ...
            # FRW vs Ea
            ax = axeses[0][1]
            x, y = np.array(list(self.get_certain_objective_function(4))) / self.rotor_weight, list(
                self.get_certain_objective_function(6))
            x = x.tolist()
            self.my_scatter_plot(x, y, O[::], fig=fig2, ax=ax, marker=marker)
            ax.set_xlabel('FRW [1]\n(b)')
            ...
            ax = axeses[1][0]
            x, y = np.array(list(self.get_certain_objective_function(4))) / self.rotor_weight, list(
                self.get_certain_objective_function(5))
            x = x.tolist()
            self.my_scatter_plot(x, y, O[::], fig=fig2, ax=ax, marker=marker)
            ax.set_xlabel('FRW [1]\n(c)')
        ...
        if False:
            ...
            x, y = np.array(list(self.get_certain_objective_function(9))) + np.array(
                list(self.get_certain_objective_function(12))) + np.array(
                list(self.get_certain_objective_function(13))), list(self.get_certain_objective_function(6))
            x = x.tolist()
    ...
    def sensitivity_bar_charts(self):
        ...
        y_label_list = ['PF', r'$\eta$ [100%]', r'$T_{em} [N]$', r'$T_{rip}$ [100%]', r'$|F|$ [N]', r'$E_m$ [100%]',
                        r'$E_a$ [deg]',
                        r'$P_{Cu,s,JMAG}$', r'$P_{Cu,r,JMAG}$', r'$P_{Fe}$ [W]', r'$P_{eddy}$', r'$P_{hyst}$',
                        r'$P_{Cu,s,FEMM}$', r'$P_{Cu,r,FEMM}$',
        ...
        for ind, i in enumerate(list(range(7)) + [INDEX_TOTAL_LOSS]):
            ...
            for j in range(int(len(y) / number_of_variant)):  # iterate design parameters
                ...
                try:
                    # if j == 6:
                    ax_list[ind].plot(y_vs_design_parameter, 'o-', lw=0.75, label=str(j) + ' ' + param_list[j],
                                      alpha=0.5)
                except IndexError as e:
                    print(
                        'Check the length of y should be 7*(%d+1)=%d, or else you should remove the redundant results in swarm_data.txt (they are produced because of the interrupted/resumed script run.)' % (
                        number_of_variant, 7 * number_of_variant))
                    raise e
                print('\tj=', j, param_list[j], '\t\t Max-Min:',
                      max(y_vs_design_parameter) - min(y_vs_design_parameter))
        ...
        if self.reference_design is not None:
            ...
            O2_ref = fobj_scalar(self.reference_data[2],
                                 ...
                                 weights=use_weights('O2'), rotor_volume=self.rotor_volume,
            ...
            O1_ref = fobj_scalar(self.reference_data[2],
                                 ...
                                 self.reference_data[6],
                                 self.reference_data[INDEX_TOTAL_LOSS],
                                 weights=use_weights('O1'), rotor_volume=self.rotor_volume,
                                 rotor_weight=self.rotor_weight)
        else:
        ...
        O1 = fobj_list(list(self.get_certain_objective_function(2)),
                       ...
                       list(self.get_certain_objective_function(5)),
                       list(self.get_certain_objective_function(6)),
                       np.array(list(self.get_certain_objective_function(9))) + np.array(
                           list(self.get_certain_objective_function(12))) + np.array(
                           list(self.get_certain_objective_function(13))),
        ...
        for j in range(int(len(O1) / number_of_variant)):  # iterate design parameters
            ...
            print('\t', j, param_list[j], '\t\t max O1 - min O1:',
                  max(O1_vs_design_parameter) - min(O1_vs_design_parameter), '\t\t', end=' ')

            # narrow bounds (refine bounds)
            results_for_refining_bounds['O1'].append(
                [ind for ind, el in enumerate(O1_vs_design_parameter) if el < O1_ref * 1.0])
        ...
        for j in range(int(len(O2) / number_of_variant)):  # iterate design parameters: range(7)
            ...
            print('\t', j, param_list[j], '\t\t max O2 - min O2:',
                  max(O2_vs_design_parameter) - min(O2_vs_design_parameter), '\t\t', end=' ')
            results_for_refining_bounds['O2'].append(
                [ind for ind, el in enumerate(O2_vs_design_parameter) if el < O2_ref * 1.0])
        ...
        O2_ecce_ax.annotate('Lower bound', xytext=(0.5, 5.5), xy=(0, 4), xycoords='data',
                            arrowprops=dict(arrowstyle="->"))
        O2_ecce_ax.annotate('Upper bound', xytext=(18.0, 5.5), xy=(20, 4), xycoords='data',
                            arrowprops=dict(arrowstyle="->"))
        ...
        rects1 = ax.bar(count - 3 * width / 8, y_min_vs_design_parameter_0, width / 8, alpha=0.5,
                        label=r'$L_g$, Air gap length', color='#6593F5')
        rects2 = ax.bar(count - 2 * width / 8, y_min_vs_design_parameter_1, width / 8, alpha=0.5,
                        label=r'$b_{st}$, Stator tooth width',
                        color='#1D2951')  # https://digitalsynopsis.com/design/beautiful-color-palettes-combinations-schemes/
        rects3 = ax.bar(count - 1 * width / 8, y_min_vs_design_parameter_2, width / 8, alpha=0.5,
                        label=r'$b_{rt}$, Rotor tooth width', color='#03396c')
        rects4 = ax.bar(count - 0 * width / 8, y_min_vs_design_parameter_3, width / 8, alpha=0.5,
                        label=r'$\theta_{so}$, Stator open width', color='#6497b1')
        rects5 = ax.bar(count + 1 * width / 8, y_min_vs_design_parameter_4, width / 8, alpha=0.5,
                        label=r'$w_{ro}$, Rotor open width', color='#0E4D92')
        rects6 = ax.bar(count + 2 * width / 8, y_min_vs_design_parameter_5, width / 8, alpha=0.5,
                        label=r'$d_{so}$, Stator open depth', color='#005b96')
        rects7 = ax.bar(count + 3 * width / 8, y_min_vs_design_parameter_6, width / 8, alpha=0.5,
                        label=r'$d_{ro}$, Rotor open depth', color='#b3cde0')
        ...
        ax.plot(rects4[0].get_x() + 0.5 * width * minus_one_one, ref[0] * one_one, 'k--', lw=1.0, alpha=0.6,
        ...
        ax.text(rects4[0].get_x() - 3.5 / 8 * width, ref[0] * 1.01, '%.2f' % (ref[0]), ha='center', va='bottom',
                rotation=90, **textfont)
        ax.text(rects4[1].get_x() - 3.5 / 8 * width, ref[1] * 1.01, '%.2f' % (ref[1]), ha='center', va='bottom',
                rotation=90, **textfont)
        ax.text(rects4[2].get_x() - 3.5 / 8 * width, ref[2] * 1.01, '%.2f' % (ref[2]), ha='center', va='bottom',
                rotation=90, **textfont)
        ax.text(rects4[3].get_x() - 3.5 / 8 * width, ref[3] * 1.01, '%.2f' % (ref[3]), ha='center', va='bottom',
                rotation=90, **textfont)
        ax.text(rects4[4].get_x() - 3.5 / 8 * width, ref[4] * 1.01, '%.2f' % (ref[4]), ha='center', va='bottom',
                rotation=90, **textfont)
        ax.text(rects4[5].get_x() - 3.5 / 8 * width, ref[5] * 1.01, '%.2f' % (ref[5]), ha='center', va='bottom',
                rotation=90, **textfont)
        ax.text(rects4[6].get_x() - 3.5 / 8 * width, ref[6] * 1.01, '%.2f' % (ref[6]), ha='center', va='bottom',
                rotation=90, **textfont)
        ax.text(rects4[7].get_x() - 3.5 / 8 * width, ref[7] * 1.01, '%.2f' % (ref[7]), ha='center', va='bottom',
                rotation=90, **textfont)

        rects1 = ax.bar(count - 3 * width / 8, y_max_vs_design_parameter_0, width / 8, alpha=0.5,
                        label=r'$L_g$,         Air gap length', color='#6593F5')  # bottom=y_min_vs_design_parameter_0,
        rects2 = ax.bar(count - 2 * width / 8, y_max_vs_design_parameter_1, width / 8, alpha=0.5,
                        label=r'$b_{st}$, Stator tooth width', color='#1D2951')  # bottom=y_min_vs_design_parameter_1,
        rects3 = ax.bar(count - 1 * width / 8, y_max_vs_design_parameter_2, width / 8, alpha=0.5,
                        label=r'$b_{rt}$, Rotor tooth width', color='#03396c')  # bottom=y_min_vs_design_parameter_2,
        rects4 = ax.bar(count - 0 * width / 8, y_max_vs_design_parameter_3, width / 8, alpha=0.5,
                        label=r'$\theta_{so}$, Stator open width',
                        color='#6497b1')  # bottom=y_min_vs_design_parameter_3,
        rects5 = ax.bar(count + 1 * width / 8, y_max_vs_design_parameter_4, width / 8, alpha=0.5,
                        label=r'$w_{ro}$, Rotor open width', color='#0E4D92')  # bottom=y_min_vs_design_parameter_4,
        rects6 = ax.bar(count + 2 * width / 8, y_max_vs_design_parameter_5, width / 8, alpha=0.5,
                        label=r'$d_{so}$, Stator open depth', color='#005b96')  # bottom=y_min_vs_design_parameter_5,
        rects7 = ax.bar(count + 3 * width / 8, y_max_vs_design_parameter_6, width / 8, alpha=0.5,
                        label=r'$d_{ro}$, Rotor open depth', color='#b3cde0')  # bottom=y_min_vs_design_parameter_6,
    ...
    if False:
        ...
        if resolution > targetFreq:
            ...
            if True:  # for half period signal
                ...
                print(
                    'Periodical extension is applied. This will not really increase your resolution. It is a visual trick for Fourier Analysis.')
        ...
        print("------------------------")
        print("\tPhase Difference of I (version 1): %g\n" % ((gs_i.phase - gs_u.phase) * 180 / math.pi), 'PF=',
              cos(gs_i.phase - gs_u.phase))
        ...
        print("\tPhase Difference of I (version 2): %g\n" % (arctan2(Iimag, Ireal) * 180 / math.pi), 'PF=',
              cos(arctan2(Iimag, Ireal)))
