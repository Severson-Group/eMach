...
class BSPM_EM_Analysis():
    ...
    def analyze(self, problem, counter = 0):
        ...
        if self.configuration['delete_results_after_calculation'] == False:
            ...
            app.GetDataManager().GetGraphModel("Circuit Voltage").WriteTable(self.configuration['JMAG_csv_folder'] + self.project_name + "_EXPORT_CIRCUIT_VOLTAGE.csv")
        toolJd.close()
        ####################################################
        # 03 Load FEA output
        ####################################################
    ...
    def excitation_freq(self):
        return self.operating_point.speed * self.machine_variant.p/(2*np.pi)

    @property
    def l_coil(self):
        tau_u  = (2*np.pi/self.machine_variant.Q) * (self.machine_variant.r_si + self.machine_variant.d_sp + self.machine_variant.d_st/2)
        l_ew   = np.pi * 0.5 * (tau_u + self.machine_variant.w_st)/2 + tau_u * self.machine_variant.Kov * (self.machine_variant.pitch - 1)
    ...
    def pre_process(self, app, model):
        ...
        partIDRange_Coil = part_ID_list[int(2+self.machine_variant.p*2)+2 : int(2+self.machine_variant.p*2)+2 + int(self.machine_variant.Q*2)]
    ...
    def add_magnetic_transient_study(self, app, model, dir_csv_output_folder, study_name):
        ...
        study.GetCondition("RotCon").SetValue(u"InitialRotationAngle", -self.machine_variant.alpha_m*0.5 + 90 + self.initial_excitation_bias_compensation_deg() + (180/self.machine_variant.p)) # add 360/(2p) deg to reverse the initial magnetizing direction to make torque positive.
        ...
        if True:
            ...
            cond.SetValue("StartReferenceStep", number_of_total_steps+1-number_of_steps_2TS*0.5) # 1/4 period <=> number_of_steps_2TS*0.5
    ...
    def add_mesh(self, study, model):
        ...
        study.GetMeshControl().SetValue("RadialDivision", self.configuration['mesh_radial_division']) # for air region near which motion occurs
        study.GetMeshControl().SetValue("CircumferentialDivision", self.configuration['mesh_circum_division']) #1440) # for air region near which motion occurs 这个数足够大，sliding mesh才准确。
        study.GetMeshControl().SetValue("AirRegionScale", self.configuration['mesh_air_region_scale']) # [Model Length]: Specify a value within the following area. (1.05 <= value < 1000)
        ...
        study.GetMeshControl().GetCondition("MagnetMeshCtrl").SetValue("Size", self.configuration['mesh_magnet_size']*1e3) # mm
    ...
    def create_custom_material(self, app, steel_name):
        ...
        app.GetMaterialLibrary().CreateCustomMaterial(self.machine_variant.stator_iron_mat['core_material'], "Custom Materials")
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("Density", self.machine_variant.stator_iron_mat['core_material_density'])
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("MagneticSteelPermeabilityType", 2)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("CoerciveForce", 0)
        # app.GetMaterialLibrary().GetUserMaterial(u"Arnon5-final").GetTable("BhTable").SetName(u"SmoothZeroPointOne")
        BH = np.loadtxt(self.machine_variant.stator_iron_mat['core_bh_file'], unpack=True, usecols=(0,1)) # values from Nishanth Magnet BH curve
        refarray = BH.T.tolist()
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).GetTable("BhTable").SetTable(refarray)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("DemagnetizationCoerciveForce", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("MagnetizationSaturated", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("MagnetizationSaturated2", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("ExtrapolationMethod", 1)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("YoungModulus", self.machine_variant.stator_iron_mat['core_youngs_modulus'])
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("ShearModulus", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("YoungModulusX", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("YoungModulusY", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("YoungModulusZ", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("ShearModulusXY", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("ShearModulusYZ", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("ShearModulusZX", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G11", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G12", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G13", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G14", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G15", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G16", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G22", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G23", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G24", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G25", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G26", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G33", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G34", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G35", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G36", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G44", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G45", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G46", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G55", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G56", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("G66", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("MagnetizationSaturated2", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("MagnetizationSaturatedMakerValue", 0)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("Loss_Type", 1)
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("LossConstantKhX", self.machine_variant.stator_iron_mat['core_ironloss_Kh'])
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("LossConstantKeX", self.machine_variant.stator_iron_mat['core_ironloss_Ke'])
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("LossConstantAlphaX", self.machine_variant.stator_iron_mat['core_ironloss_a'])
        app.GetMaterialLibrary().GetUserMaterial(self.machine_variant.stator_iron_mat['core_material']).SetValue("LossConstantBetaX", self.machine_variant.stator_iron_mat['core_ironloss_b'])
    ...
    def add_material(self, study):       
        ...
        study.GetMaterial("StatorCore").SetValue("LaminationFactor", self.machine_variant.stator_iron_mat['core_stacking_factor'])
        ...
        study.GetMaterial("NotchedRotor").SetValue("LaminationFactor", self.machine_variant.stator_iron_mat['core_stacking_factor'])
        ...
        study.GetMaterial(u"Magnet").SetValue(u"Temperature", self.machine_variant.magnet_mat['magnet_max_temperature']) # TEMPERATURE (There is no 75 deg C option)
    ...
    def add_circuit(self, app, model, study, bool_3PhaseCurrentSource=True):
        ...
        def circuit(poles,turns,Rs,ampT,ampS,freq, x=10,y=10):
            ...
            f1 = app.FunctionFactory().Sin(ampT, freq, 0) # "freq" variable cannot be used here. So pay extra attension here when you create new case of a different freq.
            func.AddFunction(f1)
            study.GetCircuit().GetComponent(I1t).SetFunction(func)

            func = app.FunctionFactory().Composite()        
            f1 = app.FunctionFactory().Sin(ampT, freq, -120) # "freq" variable cannot be used here. So pay extra attension here when you create new case of a different freq.
            func.AddFunction(f1)
            study.GetCircuit().GetComponent(I2t).SetFunction(func)

            func = app.FunctionFactory().Composite()        
            f1 = app.FunctionFactory().Sin(ampT, freq, -240) # "freq" variable cannot be used here. So pay extra attension here when you create new case of a different freq.
            func.AddFunction(f1)
            study.GetCircuit().GetComponent(I3t).SetFunction(func)

            func = app.FunctionFactory().Composite()        
            f1 = app.FunctionFactory().Sin(ampS, freq, 0) # "freq" variable cannot be used here. So pay extra attension here when you create new case of a different freq.
            f2 = app.FunctionFactory().Sin(-ampT/2, freq, 0) # "freq" variable cannot be used here. So pay extra attension here when you create new case of a different freq.
            func.AddFunction(f1)
            func.AddFunction(f2)
            study.GetCircuit().GetComponent(I1s).SetFunction(func)

            func = app.FunctionFactory().Composite()        
            f1 = app.FunctionFactory().Sin(ampS, freq, 120) # "freq" variable cannot be used here. So pay extra attension here when you create new case of a different freq.
            f2 = app.FunctionFactory().Sin(-ampT/2, freq, -120) # "freq" variable cannot be used here. So pay extra attension here when you create new case of a different freq.
            func.AddFunction(f1)
            func.AddFunction(f2)
            study.GetCircuit().GetComponent(I2s).SetFunction(func)

            func = app.FunctionFactory().Composite()        
            f1 = app.FunctionFactory().Sin(ampS, freq, 240) # "freq" variable cannot be used here. So pay extra attension here when you create new case of a different freq.
            f2 = app.FunctionFactory().Sin(-ampT/2, freq, -240) # "freq" variable cannot be used here. So pay extra attension here when you create new case of a different freq.
            func.AddFunction(f1)
            func.AddFunction(f2)
        ...
        slot_area_utilizing_ratio = (current_tpeak/2 + current_speak) / (self.machine_variant.Rated_current*np.sqrt(2))
        ...
        circuit(self.machine_variant.p, self.machine_variant.Z_q, Rs=self.R_coil, ampT= current_tpeak, ampS= current_speak, freq=self.excitation_freq)


        for suffix, poles in zip(['a', 'b'], [self.machine_variant.p*2, self.machine_variant.ps*2]): # 仍然需要考虑poles，是因为为Coil设置Set那里的代码还没有更新。这里的2(self.DriveW_poles)和4(self.BeariW_poles)等价于leftlayer和rightlayer。
            for UVW in ['U','V','W']:
        ...
        for UVW, UpDown in zip(self.machine_variant.layer_phases[0], self.machine_variant.layer_polarity[0]):
            ...
            subcondition.AddSet(model.GetSetList().GetSet("coil_%s%s%s %d"%('right_',UVW,UpDown,count)), 0) # right layer
            ...
            subcondition.ClearParts()
            subcondition.AddSet(model.GetSetList().GetSet("coil_%s%s%s %d"%('left_',UVW,UpDown,count_leftlayer)), 0) # left layer
            subcondition.SetValue("Direction2D", dict_dir[UpDown])
            index += 1
        # clean up
            for suffix in ['a', 'b']:
