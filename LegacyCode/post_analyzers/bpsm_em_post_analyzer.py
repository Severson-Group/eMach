...
class BSPM_EM_PostAnalyzer():
    ...
    def get_next_state(results, in_state):
        ...
        # d_mp    = machine.d_mp
        # p       = machine.p
        # volumes['V_rfe'] = np.pi*((r_ro-d_m)**2-r_sh**2)*l_st + (np.pi - p*alpha_m)*((r_ro-d_mp)**2 - (r_ro-d_m)**2)*l_st
        ...
        post_processing['power_factor'] = compute_power_factor(results['voltage'],
                                        results['current'],target_freq=machine.mech_omega*machine.p/(2*np.pi))
        
        
        state_out.conditions.update(post_processing) 
        return state_out
...
def compute_power_factor(voltage_df, current_df, target_freq, numPeriodicalExtension=1000):
    ...
    power_factor = compute_power_factor_from_half_period(voltage, current, mytime, targetFreq=target_freq, numPeriodicalExtension=numPeriodicalExtension)
    return power_factor
