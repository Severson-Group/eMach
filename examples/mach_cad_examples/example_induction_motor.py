...
# from eMach.mach_cad.model_obj.dimensions.dim_radian import DimRadian
...
toolFEMM.newdocument(1, 0)
toolFEMM.probdef(0, 'millimeters', 'planar', 1e-8, 1, 30, 1)
# Add materials
toolFEMM.add_material("Air")
toolFEMM.add_material("Copper")
toolFEMM.add_new_material(mat_name=bim_materials["rotor_bar_mat"]["bar_material"])
hdata, bdata = np.loadtxt(bim_materials["stator_iron_mat"]['core_bh_file'], unpack=True, usecols=(0, 1))
toolFEMM.add_new_material(mat_name=bim_materials["stator_iron_mat"]["core_material"],hdata=hdata,bdata=bdata)

# Draw the model
stator_tool = comp_stator.make(toolFEMM, toolFEMM)
winding_tool1 = []
for i in range(0,Q):
    winding_tool1.append(comp_winding_layer1[i].make(toolFEMM, toolFEMM))
winding_tool2 = []
for i in range(0,Q):
    winding_tool2.append(comp_winding_layer2[i].make(toolFEMM, toolFEMM))
rotor_tool = comp_rotor.make(toolFEMM, toolFEMM)
bar_tool = []
for i in range(0,Qr):
    bar_tool.append(comp_bar[i].make(toolFEMM, toolFEMM))
...
toolFEMM.create_boundary_condition(number_of_shells=7, radius=radius_boundary, centerxy=(0,0), bc=1)
# Save the file
toolFEMM.save_as("example_induction_motor.fem")
...
# Draw the model
stator_tool = comp_stator.make(toolJMAG, toolJMAG)
winding_tool1 = []
for i in range(0,Q):
    winding_tool1.append(comp_winding_layer1[i].make(toolJMAG, toolJMAG))
winding_tool2 = []
for i in range(0,Q):
    winding_tool2.append(comp_winding_layer2[i].make(toolJMAG, toolJMAG))
rotor_tool = comp_rotor.make(toolJMAG, toolJMAG)
bar_tool = []
for i in range(0,Qr):
    bar_tool.append(comp_bar[i].make(toolJMAG, toolJMAG))
# Save the file
toolJMAG.save()