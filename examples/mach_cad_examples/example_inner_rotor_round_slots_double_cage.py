...
rotor1 = mo.CrossSectInnerRotorRoundSlotsDoubleCage(
    name="RotorCore",
...
bar1_tool = []
for i in range(0,Qr):
    bar1_tool.append(comp2[i].make(toolFEMM, toolFEMM))
bar2_tool = []
for i in range(0,Qr):
    bar2_tool.append(comp3[i].make(toolFEMM, toolFEMM))
toolFEMM.save_as("inner_rotor_round_slots_double_cage.fem")
...
bar1_tool = []
for i in range(0,Qr):
    bar1_tool.append(comp2[i].make(toolJMAG, toolJMAG))
bar2_tool = []
for i in range(0,Qr):
    bar2_tool.append(comp3[i].make(toolJMAG, toolJMAG))
toolJMAG.save()