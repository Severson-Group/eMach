...
class ExampleArchitect(me.Architect):
    ...
    def create_new_design(self, x) -> "me.Machine":
        ...
        design_param_dict["r_sh"] = design_param_dict["r_ro"] - design_param_dict["d_m"]
        design_param_dict["r_so"] = (
            design_param_dict["r_ro"]
...
class ExampleObjective(me.Objective):
    def getObjectives(self, fullResults) -> "tuple":
        ...
        objs = (-finalState.stateConditions.P, -finalState.stateConditions.P_density)
        return objs
...
if __name__ == "__main__":
    ...
    evalSteps = [
        me.AnalysisStep(
            MagneticLoadingProblemDefinition(),
            MagneticLoadingAnalyzer(),
            MagneticLoadingPostAnalyzer(),
        ),
        me.AnalysisStep(OmegaProblemDefinition(), OmegaAnalyzer(), OmegaPostAnalyzer()),
        me.AnalysisStep(
            LengthProblemDefinition(), LengthAnalyzer(), LengthPostAnalyzer()
        ),
        me.AnalysisStep(
            TorqueProblemDefinition(), TorqueAnalyzer(), TorquePostAnalyzer()
        ),
        me.AnalysisStep(PowerProblemDefinition(), PowerAnalyzer(), PowerPostAnalyzer()),
        me.AnalysisStep(LossProblemDefinition(), LossAnalyzer(), LossPostAnalyzer()),
        me.AnalysisStep(
            EfficiencyProblemDefinition(),
            EfficiencyAnalyzer(),
            EfficiencyPostAnalyzer(),
        ),
        me.AnalysisStep(
            PowerDensityProblemDefinition(),
    ...
    bounds = ([0.02, 0.009, 0.001, 0.01], [0.5, 0.015, 0.01, 0.4])
    # Number of objectives
    n_obj = 2
    """Create the MachineDesignProblem Object"""
    machDesProb = me.MachineDesignProblem(des, evaluator, objectives, dh, bounds, n_obj)
